...
UKF::UKF()
  ...
  lidar_direction_r_cv_ =
      Eigen::MatrixXd(num_lidar_direction_state_, num_lidar_direction_state_);
  lidar_direction_r_ctrv_ =
      Eigen::MatrixXd(num_lidar_direction_state_, num_lidar_direction_state_);
  lidar_direction_r_rm_ =
      Eigen::MatrixXd(num_lidar_direction_state_, num_lidar_direction_state_);

  k_lidar_direction_cv_ =
      Eigen::MatrixXd(num_state_, num_lidar_direction_state_);
  k_lidar_direction_ctrv_ =
      Eigen::MatrixXd(num_state_, num_lidar_direction_state_);
  k_lidar_direction_rm_ =
      Eigen::MatrixXd(num_state_, num_lidar_direction_state_);
...
void UKF::initialize(const Eigen::VectorXd& z, const double timestamp,
  ...
  // init covariance matrix by hardcoding since no clue about initial state
  // covrariance
  p_merge_ << 0.5, 0, 0, 0, 0, 0, 0.5, 0, 0, 0, 0, 0, 3, 0, 0, 0, 0, 0, 10, 0,
      0, 0, 0, 0, 1;

  // set weights
  // reference from "The Unscented Kalman Filter for Nonlinear Estimation, Eric
  // A. Wan and Rudolph van der Merwe, 2000" alpha = 0.0025, beta = 2, k = 0
  double alpha = 0.0025;
  double beta = 2;
  double k = 0;
  ...
  double weight_c_0 =
      lambda_ / (lambda_ + num_state_) + (1 - alpha * alpha + beta);
...
void UKF::updateModeProb(const std::vector<double>& lambda_vec) {
  ...
  double sumGauss = cvGauss * mode_prob_cv_ + ctrvGauss * mode_prob_ctrv_ +
                    rmGauss * mode_prob_rm_;
...
void UKF::mergeEstimationAndCovariance() {
  x_merge_ =
      mode_prob_cv_ * x_cv_ + mode_prob_ctrv_ * x_ctrv_ + mode_prob_rm_ * x_rm_;
  ...
  p_merge_ =
      mode_prob_cv_ *
          (p_cv_ + (x_cv_ - x_merge_) * (x_cv_ - x_merge_).transpose()) +
      mode_prob_ctrv_ *
          (p_ctrv_ + (x_ctrv_ - x_merge_) * (x_ctrv_ - x_merge_).transpose()) +
      mode_prob_rm_ *
          (p_rm_ + (x_rm_ - x_merge_) * (x_rm_ - x_merge_).transpose());
}

void UKF::mixingProbability() {
  double sumProb1 = mode_prob_cv_ * p1_[0] + mode_prob_ctrv_ * p2_[0] +
                    mode_prob_rm_ * p3_[0];
  double sumProb2 = mode_prob_cv_ * p1_[1] + mode_prob_ctrv_ * p2_[1] +
                    mode_prob_rm_ * p3_[1];
  double sumProb3 = mode_prob_cv_ * p1_[2] + mode_prob_ctrv_ * p2_[2] +
                    mode_prob_rm_ * p3_[2];
...
void UKF::interaction() {
  ...
  x_cv_ = mode_match_prob_cv2cv_ * x_pre_cv +
          mode_match_prob_ctrv2cv_ * x_pre_ctrv +
          mode_match_prob_rm2cv_ * x_pre_rm;
  x_ctrv_ = mode_match_prob_cv2ctrv_ * x_pre_cv +
            mode_match_prob_ctrv2ctrv_ * x_pre_ctrv +
            mode_match_prob_rm2ctrv_ * x_pre_rm;
  x_rm_ = mode_match_prob_cv2rm_ * x_pre_cv +
          mode_match_prob_ctrv2rm_ * x_pre_ctrv +
          mode_match_prob_rm2rm_ * x_pre_rm;
  ...
  p_cv_ = mode_match_prob_cv2cv_ *
              (p_pre_cv + (x_pre_cv - x_cv_) * (x_pre_cv - x_cv_).transpose()) +
          mode_match_prob_ctrv2cv_ *
              (p_pre_ctrv +
               (x_pre_ctrv - x_cv_) * (x_pre_ctrv - x_cv_).transpose()) +
          mode_match_prob_rm2cv_ *
              (p_pre_rm + (x_pre_rm - x_cv_) * (x_pre_rm - x_cv_).transpose());
  p_ctrv_ =
      mode_match_prob_cv2ctrv_ *
          (p_pre_cv + (x_pre_cv - x_ctrv_) * (x_pre_cv - x_ctrv_).transpose()) +
      mode_match_prob_ctrv2ctrv_ *
          (p_pre_ctrv +
           (x_pre_ctrv - x_ctrv_) * (x_pre_ctrv - x_ctrv_).transpose()) +
      mode_match_prob_rm2ctrv_ *
          (p_pre_rm + (x_pre_rm - x_ctrv_) * (x_pre_rm - x_ctrv_).transpose());
  p_rm_ = mode_match_prob_cv2rm_ *
              (p_pre_cv + (x_pre_cv - x_rm_) * (x_pre_cv - x_rm_).transpose()) +
          mode_match_prob_ctrv2rm_ *
              (p_pre_ctrv +
               (x_pre_ctrv - x_rm_) * (x_pre_ctrv - x_rm_).transpose()) +
          mode_match_prob_rm2rm_ *
...
void UKF::updateEachMotion(
  ...
  double b = 2 * num_meas * (1 - detection_probability * gate_probability) /
             (gating_thres * detection_probability);
  ...
  for (int motion_ind = 0; motion_ind < num_motion_model_; motion_ind++) {
    ...
    for (size_t i = 0; i < num_meas; i++) {
      ...
      if (is_direction_available) meas(2) = object_vec[i].angle;
    ...
    if (num_meas != 0) {
      std::vector<double>::iterator max_iter =
          std::max_element(e_vec.begin(), e_vec.end());
    ...
    for (size_t i = 0; i < num_meas; i++) {
      sigma_p += (beta_vec[i] * diff_vec[i] * diff_vec[i].transpose() -
                  sigma_x * sigma_x.transpose());
    }
    ...
    if (num_meas != 0) {
      updated_p = beta_zero * p +
                  (1 - beta_zero) *
                      (p - kalman_gain * s_pred * kalman_gain.transpose()) +
...
void UKF::updateMeasurementForCTRV(
    const std::vector<tracking_msgs::DetectedObject>& object_vec) {
  ...
  for (auto const& object : object_vec) {
    ...
    if (is_direction_ctrv_available_) {
      ...
      double e_ctrv = exp(-0.5 * diff_ctrv.transpose() *
                          s_lidar_direction_ctrv_.inverse() * diff_ctrv);
      ...
      double e_ctrv =
          exp(-0.5 * diff_ctrv.transpose() * s_ctrv_.inverse() * diff_ctrv);
      e_ctrv_vec.push_back(e_ctrv);
    }
  }
  std::vector<double>::iterator max_ctrv_iter =
      std::max_element(e_ctrv_vec.begin(), e_ctrv_vec.end());
...
void UKF::uppateForCTRV() {
  ...
  if (is_direction_ctrv_available_) {
    x_ctrv_.col(0) =
        x + k_lidar_direction_ctrv_ *
                (lidar_direction_ctrv_meas_ - z_pred_lidar_direction_ctrv_);
    p_ctrv_ = p_ctrv_ - k_lidar_direction_ctrv_ * s_lidar_direction_ctrv_ *
                            k_lidar_direction_ctrv_.transpose();
...
void UKF::updateIMMUKF(
  ...
  std::vector<double> lambda_vec;
  updateEachMotion(detection_probability, gate_probability, gating_thres,
                   object_vec, lambda_vec);
  /*****************************************************************************
   *  IMM Merge Step
   ****************************************************************************/
...
void UKF::initCovarQs(const double dt, const double yaw) {
  ...
  q_cv_ << 0.5 * 0.5 * dt_4 * cos_2_yaw * cv_var_a,
      0.5 * 0.5 * dt_4 * cos_sin * cv_var_a, 0.5 * dt_3 * cos_yaw * cv_var_a, 0,
      0, 0.5 * 0.5 * dt_4 * cos_sin * cv_var_a,
      0.5 * 0.5 * dt_4 * sin_2_yaw * cv_var_a, 0.5 * dt_3 * sin_yaw * cv_var_a,
      0, 0, 0.5 * dt_3 * cos_yaw * cv_var_a, 0.5 * dt_3 * sin_yaw * cv_var_a,
      dt_2 * cv_var_a, 0, 0, 0, 0, 0, 0.5 * 0.5 * dt_4 * cv_var_yawdd,
      0.5 * dt_3 * cv_var_yawdd, 0, 0, 0, 0.5 * dt_3 * cv_var_yawdd,
      dt_2 * cv_var_yawdd;
  q_ctrv_ << 0.5 * 0.5 * dt_4 * cos_2_yaw * ctrv_var_a,
      0.5 * 0.5 * dt_4 * cos_sin * ctrv_var_a,
      0.5 * dt_3 * cos_yaw * ctrv_var_a, 0, 0,
      0.5 * 0.5 * dt_4 * cos_sin * ctrv_var_a,
      0.5 * 0.5 * dt_4 * sin_2_yaw * ctrv_var_a,
      0.5 * dt_3 * sin_yaw * ctrv_var_a, 0, 0,
      0.5 * dt_3 * cos_yaw * ctrv_var_a, 0.5 * dt_3 * sin_yaw * ctrv_var_a,
      dt_2 * ctrv_var_a, 0, 0, 0, 0, 0, 0.5 * 0.5 * dt_4 * ctrv_var_yawdd,
      0.5 * dt_3 * ctrv_var_yawdd, 0, 0, 0, 0.5 * dt_3 * ctrv_var_yawdd,
      dt_2 * ctrv_var_yawdd;
  q_rm_ << 0.5 * 0.5 * dt_4 * cos_2_yaw * rm_var_a,
      0.5 * 0.5 * dt_4 * cos_sin * rm_var_a, 0.5 * dt_3 * cos_yaw * rm_var_a, 0,
      0, 0.5 * 0.5 * dt_4 * cos_sin * rm_var_a,
      0.5 * 0.5 * dt_4 * sin_2_yaw * rm_var_a, 0.5 * dt_3 * sin_yaw * rm_var_a,
      0, 0, 0.5 * dt_3 * cos_yaw * rm_var_a, 0.5 * dt_3 * sin_yaw * rm_var_a,
      dt_2 * rm_var_a, 0, 0, 0, 0, 0, 0.5 * 0.5 * dt_4 * rm_var_yawdd,
      0.5 * dt_3 * rm_var_yawdd, 0, 0, 0, 0.5 * dt_3 * rm_var_yawdd,
      dt_2 * rm_var_yawdd;
}
...
void UKF::predictionMotion(const double delta_t, const int model_ind) {
  ...
  for (int i = 0; i < num_state_; i++) {
    ...
    while (pred1(3) > M_PI) pred1(3) -= 2. * M_PI;
    while (pred1(3) < -M_PI) pred1(3) += 2. * M_PI;

    while (pred2(3) > M_PI) pred2(3) -= 2. * M_PI;
    while (pred2(3) < -M_PI) pred2(3) += 2. * M_PI;
...
void UKF::updateKalmanGain(const int motion_ind) {
  ...
  Eigen::MatrixXd cross_covariance =
      Eigen::MatrixXd(num_state_, num_meas_state);
  ...
  for (int i = 0; i < 2 * num_state_ + 1; i++) {
    ...
    cross_covariance =
        cross_covariance + weights_c_(i) * x_diff * z_diff.transpose();
  }
...
void UKF::predictionLidarMeasurement(const int motion_ind,
...
double UKF::calculateNIS(const tracking_msgs::DetectedObject& in_object,
...
bool UKF::isLaneDirectionAvailable(
    const tracking_msgs::DetectedObject& in_object, const int motion_ind,
...
void UKF::checkLaneDirectionAvailability(
    const tracking_msgs::DetectedObject& in_object,
    const double lane_direction_chi_thres, const bool use_sukf) {
  if (use_sukf) {
    is_direction_ctrv_available_ = isLaneDirectionAvailable(
        in_object, MotionModel::CTRV, lane_direction_chi_thres);
  } else {
    is_direction_cv_available_ = isLaneDirectionAvailable(
        in_object, MotionModel::CV, lane_direction_chi_thres);
    is_direction_ctrv_available_ = isLaneDirectionAvailable(
        in_object, MotionModel::CTRV, lane_direction_chi_thres);
  }
}
...
void UKF::update(const bool use_sukf, const double detection_probability,
                 const double gate_probability, const double gating_thres,
                 const std::vector<tracking_msgs::DetectedObject>& object_vec) {
  if (use_sukf) {
    updateSUKF(object_vec);
  } else {
    updateIMMUKF(detection_probability, gate_probability, gating_thres,
                 object_vec);
  }
}
