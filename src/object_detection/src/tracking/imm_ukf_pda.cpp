...
ImmUkfPda::ImmUkfPda()
  ...
  lane_direction_chi_thres_= 2.71;
  nearest_lane_distance_thres_= 1.0;
...
//   private_nh_.param<double>("detection_probability", detection_probability_,
//                             0.9);
//   private_nh_.param<double>("static_velocity_thres", static_velocity_thres_,
//                             0.5);
//   private_nh_.param<int>("static_velocity_history_thres",
//                          static_num_history_thres_, 3);
//   private_nh_.param<double>("prevent_explosion_thres", prevent_explosion_thres_,
//                             1000);
//   private_nh_.param<double>("merge_distance_threshold",
//                             merge_distance_threshold_, 0.5);
...
//   private_nh_.param<double>("lane_direction_chi_thres",
//                             lane_direction_chi_thres_, 2.71);
//   private_nh_.param<double>("nearest_lane_distance_thres",
//                             nearest_lane_distance_thres_, 1.0);
...
void ImmUkfPda::run() {
  // pub_object_array_ =
  //     node_handle_.advertise<tracking_msgs::DetectedObjectArray>(
  //         "/detection/objects", 1);
  // sub_detected_array_ = node_handle_.subscribe(
  //     "/detection/fusion_tools/objects", 1, &ImmUkfPda::callback, this);

  // if (use_vectormap_) {
  //   vmap_.subscribe(private_nh_,
  //                   vector_map::Category::POINT | vector_map::Category::NODE
  //                   |
  //                       vector_map::Category::LANE,
  //                   1);
  // }
}

void ImmUkfPda::run_test(
  ...
  std::cout<<"output.size: "<<detected_objects_output.objects.size()<<std::endl;
}

void ImmUkfPda::callback(const tracking_msgs::DetectedObjectArray& input) {
  input_header_ = input.header;
...
void ImmUkfPda::checkVectormapSubscription() {
  ...
  //   lanes_ = vmap_.findByFilter(
  //       [](const vector_map_msgs::Lane& lane) { return true; });
  //   if (lanes_.empty()) {
...
bool ImmUkfPda::updateNecessaryTransform() {
  bool success = true;
  try {
    tf_listener_.waitForTransform(input_header_.frame_id, tracking_frame_,
                                  ros::Time(0), ros::Duration(1.0));
    tf_listener_.lookupTransform(tracking_frame_, input_header_.frame_id,
                                 ros::Time(0), local2global_);
  } catch (tf::TransformException ex) {
    ROS_ERROR("%s", ex.what());
    success = false;
  }

  // std::cout<<"local2global_: "<<local2global_.frame_id_<<std::endl;

  if (use_vectormap_ && has_subscribed_vectormap_) {
    try {
      tf_listener_.waitForTransform(vectormap_frame_, tracking_frame_,
                                    ros::Time(0), ros::Duration(1.0));
      tf_listener_.lookupTransform(vectormap_frame_, tracking_frame_,
                                   ros::Time(0), tracking_frame2lane_frame_);
      tf_listener_.lookupTransform(tracking_frame_, vectormap_frame_,
                                   ros::Time(0), lane_frame2tracking_frame_);
    } catch (tf::TransformException ex) {
      ROS_ERROR("%s", ex.what());
    }
  }
...
void ImmUkfPda::transformPoseToGlobal(
  ...
  for (auto const& object : input.objects) {
    geometry_msgs::Pose out_pose =
        getTransformedPose(object.pose, local2global_);

    tracking_msgs::DetectedObject dd;
...
void ImmUkfPda::transformPoseToLocal(
  ...
  for (auto& object : detected_objects_output.objects) {
    geometry_msgs::Pose out_pose =
        getTransformedPose(object.pose, global2local);
    object.header = input_header_;
    object.pose = out_pose;
  }
}

geometry_msgs::Pose ImmUkfPda::getTransformedPose(
    const geometry_msgs::Pose& in_pose, const tf::StampedTransform& tf_stamp) {
  tf::Transform transform;
  geometry_msgs::PoseStamped out_pose;
  transform.setOrigin(
      tf::Vector3(in_pose.position.x, in_pose.position.y, in_pose.position.z));
...
void ImmUkfPda::measurementValidation(
  ...
  // measurement if making it allows to have more than one measurement, you will
  // see non semipositive definite covariance
  ...
  if (exists_smallest_nis_object) {
    ...
    // if (use_vectormap_ && has_subscribed_vectormap_) {
    //   tracking_msgs::DetectedObject direction_updated_object;
    ...
    //   object_vec.push_back(target.object_);
    // }
  }  // end for if (exists_smallest_nis_object)
}
...
bool ImmUkfPda::storeObjectWithNearestLaneDirection(
    const tracking_msgs::DetectedObject& in_object,
    tracking_msgs::DetectedObject& out_object) {
  // geometry_msgs::Pose lane_frame_pose =
  //     getTransformedPose(in_object.pose, tracking_frame2lane_frame_);
  // double min_dist = std::numeric_limits<double>::max();

  // double min_yaw = 0;
  // for (auto const& lane : lanes_) {
  //   vector_map_msgs::Node node =
  //       vmap_.findByKey(vector_map::Key<vector_map_msgs::Node>(lane.bnid));
  //   vector_map_msgs::Point point =
  //       vmap_.findByKey(vector_map::Key<vector_map_msgs::Point>(node.pid));
  //   double distance =
  //       std::sqrt(std::pow(point.bx - lane_frame_pose.position.y, 2) +
  //                 std::pow(point.ly - lane_frame_pose.position.x, 2));
  //   if (distance < min_dist) {
  //     min_dist = distance;
  //     vector_map_msgs::Node front_node =
  //         vmap_.findByKey(vector_map::Key<vector_map_msgs::Node>(lane.fnid));
  //     vector_map_msgs::Point front_point = vmap_.findByKey(
  //         vector_map::Key<vector_map_msgs::Point>(front_node.pid));
  //     min_yaw =
  //         std::atan2((front_point.bx - point.bx), (front_point.ly -
  //         point.ly));
  //   }
  // }
...
void ImmUkfPda::updateTargetWithAssociatedObject(
    const std::vector<tracking_msgs::DetectedObject>& object_vec, UKF& target) {
  target.lifetime_++;
...
void ImmUkfPda::updateBehaviorState(const UKF& target, const bool use_sukf,
...
void ImmUkfPda::initTracker(const tracking_msgs::DetectedObjectArray& input,
                            double timestamp) {
  for (size_t i = 0; i < input.objects.size(); i++) {
...
void ImmUkfPda::secondInit(
    UKF& target, const std::vector<tracking_msgs::DetectedObject>& object_vec,
    double dt) {
...
bool ImmUkfPda::arePointsClose(const geometry_msgs::Point& in_point_a,
...
bool ImmUkfPda::arePointsEqual(const geometry_msgs::Point& in_point_a,
                               const geometry_msgs::Point& in_point_b) {
  return arePointsClose(in_point_a, in_point_b, CENTROID_DISTANCE);
}

bool ImmUkfPda::isPointInPool(const std::vector<geometry_msgs::Point>& in_pool,
...
tracking_msgs::DetectedObjectArray ImmUkfPda::removeRedundantObjects(
  ...
  tracking_msgs::DetectedObjectArray resulting_objects;
  ...
  for (size_t i = 0; i < matching_objects.size(); i++) {
    ...
    tracking_msgs::DetectedObject best_object;
...
void ImmUkfPda::makeOutput(
    ...
    const std::vector<bool>& matching_vec,
    tracking_msgs::DetectedObjectArray& detected_objects_output) {
  tracking_msgs::DetectedObjectArray tmp_objects;
...
void ImmUkfPda::dumpResultText(
    tracking_msgs::DetectedObjectArray& detected_objects) {
  ...
  for (size_t i = 0; i < detected_objects.objects.size(); i++) {
    ...
    // (frame_number,tracked_id, object type, truncation, occlusion, observation
    // angle, x1,y1,x2,y2, h, w, l, cx, cy, cz, yaw) x1, y1, x2, y2 are for 2D
    // bounding box. h, w, l, are for height, width, length respectively cx, cy,
    // cz are for object centroid
